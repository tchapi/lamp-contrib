...
void setup() {
	...
	// define servo limits
	SERVO_MIN[0] = 90; // forward -> down
	SERVO_MIN[1] = 20; // back -> down
	SERVO_MIN[2] = 35; // right
	SERVO_MIN[3] = 60; // down

	SERVO_MAX[0] = 165; // back -> up
	SERVO_MAX[1] = 125; // forward -> up
	SERVO_MAX[2] = 155; // left
	SERVO_MAX[3] = 112; // up
	...
	// initialize serial for bluetooth (115200) or USB (9600) or Seeedshield (38400)
	// we use the seeedstudio bluetooth shield here
	Serial.begin(38400);
	delay(100);
	// set bluetooth name
	Serial.print("\r\n+STNA=MetalLamp\r\n");
...
void loop() {
	...
	//---------------------------------------------------------
	int bluetooth_state = analogRead(BT_STATE); delay(1);
